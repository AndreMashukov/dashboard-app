import {PAGE_ONE_SEARCH, CAMPAIGN} from "@/common/apiUrls";
import axios from "axios";
import {chunk} from "lodash";
import Vue from "vue";
import {MEMBERSHIP} from "../../common/apiUrls";
...
const actions = {
  ...
  async getListMembership({commit}){
    try {
      let res = await axios.get(`${MEMBERSHIP}/list`);
      commit('handlerListMembership', [...res.data].map(e=>({_id: e._id, name: e.name})) || [])
    } catch (error) {
  ...
  async getInitList({commit, state}, {page = 0, itemsPage = 15, filter}) {
    try {
      ...
      let res = await axios.get(`${CAMPAIGN}/list?${searchUrl}`);
  ...
  async getListItem({commit, state, dispatch}, {preMaxPage = 0, nextPage = 0, itemsPage = 15, filter = null}) {
    try {
      ...
      const newData = await Promise.all([...Array(nextPage - preMaxPage).keys()].map(idx => {
        ...
        if(filter) Object.keys(filter).forEach(fil => {searchUrl += `&${fil}=${filter[`${fil}`]}`});
        return axios.get(`${CAMPAIGN}/list?${searchUrl}`);
      }));
      ...
      dispatch('alert/error', "Get list campaign fail!", {root: true});
    }
  },

  async updateItem({commit, dispatch, state}, item) {
    try {
      const result = await axios.put(`${CAMPAIGN}/${item._id}`, item)
      const index = state.listItems.findIndex(value => value._id === item._id);
      if(index > -1) commit('editItem', {index, item: result.data});
      dispatch('alert/success', "Change campaign success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Change campaign fail!", {root: true});
    }
  },
  ...
  async deleteById({commit, dispatch, state}, id) {
    try {
      ...
      dispatch('alert/success', "Delete campaign success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Delete campaign fail!", {root: true});
    }
  },

  async createItem({commit, dispatch}, item) {
    try {
      const res = await axios.post(CAMPAIGN, item);
      commit("addItem", res.data)
      dispatch('alert/success', "Create campaign success!", {root: true})
    } catch (e) {
      dispatch('alert/error', "Create campaign fail!", {root: true})
    }
  },
};
...
const mutations = {
  ...
  handlerListMembership(state, items = []) {
    state.listMembership = items
  },
