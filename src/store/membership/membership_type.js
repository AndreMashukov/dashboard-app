import {PAGE_ONE_SEARCH, MEMBERSHIP_TYPE} from "@/common/apiUrls";
...
const actions = {
  ...
  async getInitList({commit, state}, {page = 0, itemsPage = 15, filter}) {
    try {
      ...
      let res = await axios.get(`${MEMBERSHIP_TYPE}/list?${searchUrl}`);
  ...
  async getListItem({commit, state, dispatch}, {preMaxPage = 0, nextPage = 0, itemsPage = 15, filter = null}) {
    try {
      ...
      const newData = await Promise.all([...Array(nextPage - preMaxPage).keys()].map(idx => {
        ...
        return axios.get(`${MEMBERSHIP_TYPE}/list?${searchUrl}`);
      }));
      ...
      dispatch('alert/error', "Get list type of membership fail!", {root: true});
    }
  },

  async updateItem({commit, dispatch, state}, item) {
    try {
      const result = await axios.put(`${MEMBERSHIP_TYPE}/${item._id}`, item)
      const index = state.listItems.findIndex(value => value._id === item._id);
      if(index > -1) commit('editItem', {index, item: result.data});
      dispatch('alert/success', "Change type of membership success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Change type of membership fail!", {root: true});
    }
  },

  async deleteById({commit, dispatch, state}, id) {
    try {
      await axios.delete(`${MEMBERSHIP_TYPE}/${id}`)
      const idx = state.listItems.findIndex(value => value._id === id);
      if(idx > -1) commit('deleteItem', idx);
      dispatch('alert/success', "Delete type of membership success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Delete type of membership fail!", {root: true});
    }
  },

  async createItem({commit, dispatch}, item) {
    try {
      const res = await axios.post(MEMBERSHIP_TYPE, item);
      commit("addItem", res.data)
      dispatch('alert/success', "Create type of membership success!", {root: true})
    } catch (e) {
      dispatch('alert/error', "Create type of membership fail!", {root: true})
    }
  },
};
