import {PAGE_ONE_SEARCH, MEDIA_SERIES} from "@/common/apiUrls";
...
const actions = {
  ...
  async getInitList({commit, state}, {page = 0, itemsPage = 15, filter}) {
    try {
      ...
      let res = await axios.get(`${MEDIA_SERIES}/list?${searchUrl}`);
  ...
  async getListItem({commit, state, dispatch}, {preMaxPage = 0, nextPage = 0, itemsPage = 15, filter = null}) {
    try {
      ...
      const newData = await Promise.all([...Array(nextPage - preMaxPage).keys()].map(idx => {
        ...
        if(filter) Object.keys(filter).forEach(fil => {searchUrl += `&${fil}=${filter[`${fil}`]}`});
        return axios.get(`${MEDIA_SERIES}/list?${searchUrl}`);
      }));
      ...
      dispatch('alert/error', "Get list series of media fail!", {root: true});
    }
  },

  async updateItem({commit, dispatch, state}, item) {
    try {
      const result = await axios.put(`${MEDIA_SERIES}/${item._id}`, item)
      const index = state.listItems.findIndex(value => value._id === item._id);
      if(index > -1) commit('editItem', {index, item: result.data});
      dispatch('alert/success', "Change series of media success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Change series of media fail!", {root: true});
    }
  },

  async deleteById({commit, dispatch, state}, id) {
    try {
      await axios.delete(`${MEDIA_SERIES}/${id}`)
      const idx = state.listItems.findIndex(value => value._id === id);
      if(idx > -1) commit('deleteItem', idx);
      dispatch('alert/success', "Delete series of media success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Delete series of media fail!", {root: true});
    }
  },

  async createItem({commit, dispatch}, item) {
    try {
      const res = await axios.post(MEDIA_SERIES, item);
      commit("addItem", res.data)
      dispatch('alert/success', "Create series of media success!", {root: true})
    } catch (e) {
      dispatch('alert/error', "Create series of media fail!", {root: true})
    }
  },
};
