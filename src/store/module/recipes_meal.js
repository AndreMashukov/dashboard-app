import {PAGE_ONE_SEARCH, RECIPE_MEAL} from "@/common/apiUrls";
...
const actions = {
  ...
  async getInitList({commit, state}, {page = 0, itemsPage = 15}) {
    try {
      let res = await axios.get(`${RECIPE_MEAL}/list?${PAGE_ONE_SEARCH(page, itemsPage)}`);
  ...
  async getListItem({commit, state, dispatch}, {preMaxPage = 0, nextPage = 0, itemsPage = 15, status = null}) {
    try {
      ...
      const newData = await Promise.all([...Array(nextPage - preMaxPage).keys()].map(idx => {
        if(status === null) return axios.get(`${RECIPE_MEAL}/list?${PAGE_ONE_SEARCH(preMaxPage + idx + 1, itemsPage)}`);
        return axios.get(`${RECIPE_MEAL}/list?${PAGE_ONE_SEARCH(preMaxPage + idx + 1, itemsPage)}`);
      }));
  ...
  async updateItem({commit, dispatch, state}, item) {
    try {
      const result = await axios.put(`${RECIPE_MEAL}/${item._id}`, item)
      const index = state.listItems.findIndex(value => value._id === item._id);
      if(index > -1) commit('editItem', {index, item: result.data});
      dispatch('alert/success', "Change meal of recipes success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Change meal of recipes fail!", {root: true});
    }
  },

  async deleteById({commit, dispatch, state}, id) {
    try {
      await axios.delete(`${RECIPE_MEAL}/${id}`)
      const idx = state.listItems.findIndex(value => value._id === id);
      if(idx > -1) commit('deleteItem', idx);
      dispatch('alert/success', "Delete meal of recipes success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Delete meal of recipes fail!", {root: true});
    }
  },

  async createItem({commit, dispatch}, item) {
    try {
      const res = await axios.post(RECIPE_MEAL, item);
      commit("addItem", res.data)
      dispatch('alert/success', "Create meal of recipes success!", {root: true})
    } catch (e) {
      dispatch('alert/error', "Create meal of recipes fail!", {root: true})
    }
  },
};
