import {PAGE_ONE_SEARCH, INGREDIENTS_CATEGORY} from "@/common/apiUrls";
...
const actions = {
  ...
  async getInitList({commit, state}, {page = 0, itemsPage = 15}) {
    try {
      let res = await axios.get(`${INGREDIENTS_CATEGORY}/list?${PAGE_ONE_SEARCH(page, itemsPage)}`);
  ...
  async getListItem({commit, state, dispatch}, {preMaxPage = 0, nextPage = 0, itemsPage = 15, status = null}) {
    try {
      ...
      const newData = await Promise.all([...Array(nextPage - preMaxPage).keys()].map(idx => {
        if(status === null) return axios.get(`${INGREDIENTS_CATEGORY}/list?${PAGE_ONE_SEARCH(preMaxPage + idx + 1, itemsPage)}`);
        return axios.get(`${INGREDIENTS_CATEGORY}/list?${PAGE_ONE_SEARCH(preMaxPage + idx + 1, itemsPage)}`);
      }));
      ...
      dispatch('alert/error', "Get list category of ingredients fail!", {root: true});
    }
  },

  async updateItem({commit, dispatch, state}, item) {
    try {
      const result = await axios.put(`${INGREDIENTS_CATEGORY}/${item._id}`, item)
      const index = state.listItems.findIndex(value => value._id === item._id);
      if(index > -1) commit('editItem', {index, item: result.data});
      dispatch('alert/success', "Change category of ingredients success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Change category of ingredients fail!", {root: true});
    }
  },

  async deleteById({commit, dispatch, state}, id) {
    try {
      await axios.delete(`${INGREDIENTS_CATEGORY}/${id}`)
      const idx = state.listItems.findIndex(value => value._id === id);
      if(idx > -1) commit('deleteItem', idx);
      dispatch('alert/success', "Delete category of ingredients success!", {root: true});
    } catch (e) {
      dispatch('alert/error', "Delete category of ingredients fail!", {root: true});
    }
  },

  async createItem({commit, dispatch}, item) {
    try {
      const res = await axios.post(INGREDIENTS_CATEGORY, item);
      commit("addItem", res.data)
      dispatch('alert/success', "Create category of ingredients success!", {root: true})
    } catch (e) {
      dispatch('alert/error', "Create category of ingredients fail!", {root: true})
    }
  },
};
