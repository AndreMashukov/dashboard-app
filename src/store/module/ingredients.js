import {PAGE_ONE_SEARCH, INGREDIENTS, INGREDIENTS_CATEGORY} from "@/common/apiUrls";
...
const actions = {
  ...
  async getInitList({commit, state}, {page = 0, itemsPage = 15}) {
    try {
      let res = await axios.get(`${INGREDIENTS}/list?${PAGE_ONE_SEARCH(page, itemsPage)}`);
  ...
  async getListItem({commit, state, dispatch}, {preMaxPage = 0, nextPage = 0, itemsPage = 15, status = null}) {
    try {
      ...
      const newData = await Promise.all([...Array(nextPage - preMaxPage).keys()].map(idx => {
        if(status === null) return axios.get(`${INGREDIENTS}/list?${PAGE_ONE_SEARCH(preMaxPage + idx + 1, itemsPage)}`);
        return axios.get(`${INGREDIENTS}/list?${PAGE_ONE_SEARCH(preMaxPage + idx + 1, itemsPage)}`);
      }));
