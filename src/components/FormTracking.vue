...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      if (this.dataForm.type === "input") delete this.dataForm.options
      else if (this.dataForm.type === "checkbox") this.dataForm.options = [{index: 0, value: "yes"}, {
        index: 1,
        value: "no"
      },]
      else this.dataForm.options = this.dataForm.options.map((value, index) => ({index, value}))
  ...
  >
    <b-form @submit.prevent="handleSubmit" @reset="onReset" v-if="showModal">
      ...
      <div class="mb-3 mb-0" v-if="dataForm.type === 'radio'">
        <div class="row">
          <label class="col-10">Option: (*required)</label>
